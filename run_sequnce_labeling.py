...
class AVA_Sequence_labeling_Processor(DataProcessor):
    ...
    def get_predicate_labels(self):
        return ['集计','分类','维度','模糊分类','模糊维度','保留','排除','升序','降序','时间范围']
...
def main(_):
    ...
    processors = {
        "ava": AVA_Sequence_labeling_Processor,
    }
    ...
    if FLAGS.do_train:
        ...
        modelPath = os.path.join("./output/sequnce_labeling_model", "model")
        def export_input_fn():
            name_to_features = {
                ...
                "token_label_ids": tf.placeholder(tf.int64, [None, FLAGS.max_seq_length], name='token_label_ids' ),
                "predicate_label_id": tf.placeholder(tf.int64, [None], name='predicate_label_id' ),
        ...
        estimator.export_savedmodel(modelPath, export_input_fn)
