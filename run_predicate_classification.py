...
class Classification_Processor(DataProcessor):
    def __init__(self):
        self.language = "zh"
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        modelPath = os.path.join("./output/predicate_classification_model", "model")
        def export_input_fn():
            spec = {
                ...
                "label_ids": tf.placeholder(tf.int64, [None, label_length], name='label_ids' ),
                # "is_real_example": tf.placeholder(tf.int64, [None], name='is_real_example' ),
            }
            tf.logging.info(spec["input_ids"].shape)
            return tf.estimator.export.build_raw_serving_input_receiver_fn(spec)()
